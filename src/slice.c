...
int	in_bounds(t_vector hit, t_vector p_bounds, t_vector n_bounds, t_object *obj)
{
		...
		(n_bounds.s_xyzw.z == 0 || hit.s_xyzw.z > n_bounds.s_xyzw.z))
	{
		if (obj->type == PLANE)
			return (TRUE);
		else if ((p_bounds.s_xyzw.y == 0 || hit.s_xyzw.y < p_bounds.s_xyzw.y) \
		&& (n_bounds.s_xyzw.y == 0 || hit.s_xyzw.y > n_bounds.s_xyzw.y))
			return (TRUE);
	}
	return (FALSE);
}
...
int	slice(t_ray *ray_save_hits, double t[2], t_object *obj, t_ray *ray)
{
	...
	if (obj->slice_toggle == 0 || in_bounds(local_hit, obj->slice_pos, \
	obj->slice_neg, obj))
	{
